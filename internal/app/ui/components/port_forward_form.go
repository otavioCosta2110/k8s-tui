...
type OpenPortForwardFormMsg struct {
	Form       *PortForwardForm
	Breadcrumb string
}

func NewPortForwardForm(title, resourceType, resourceName, namespace string) *PortForwardForm {
	fields := []Field{
		{Name: "localPort", Placeholder: "Local Port", DefaultValue: "8080", Row: 0},
		{Name: "remotePort", Placeholder: "Remote Port", DefaultValue: "80", Row: 0},
	}
...
func (f *PortForwardForm) Init() tea.Cmd {
	return textinput.Blink
}

func (f *PortForwardForm) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	var cmd tea.Cmd
...
func (f *PortForwardForm) View() string {
	...
	resourceInfo := lipgloss.NewStyle().
		Foreground(lipgloss.Color(customstyles.HelpTextColor)).
		Background(lipgloss.Color(customstyles.BackgroundColor)).
		PaddingTop(1).
		Width(W/2 + styles.Margin).
		Render(f.resourceType + ": " + f.resourceName + " (namespace: " + f.namespace + ")")
